...
public class SQLLiteDB {
    ...
    private static final String KeyAnalysis ="select cast(value as Integer) as value,text from (SELECT count(MetaDataKey) as value,MetaDataKey as text FROM MetaData where ((Active is NULL) or (Active = 'True'))group by MetaDataKey)order by value desc limit 100";
    ...
    private static final String IgnoreKeyStatement = "Update MetaData set Active = 'False' where MetaDataKey = ?";
    ...
    public void runIgnoreKeyStatement(Connection con, String metaDataKey) throws SQLException {
        PreparedStatement st = con.prepareStatement(IgnoreKeyStatement);
        st.setString(1, metaDataKey);
        con.setAutoCommit(true);
        Statement stmt = con.createStatement();
        st.executeUpdate();
